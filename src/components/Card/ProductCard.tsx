    ...
    return (
        <article className='border bg-white border-gray-200 rounded-sm flex flex-col justify-between shadow-sm hover:shadow-md transition-all duration-200 h-full lg:max-h-[390px] max-w-[400px]'>
            <Link to={`/customer/product-details/${item?._id}`}>
                <img
                    className='lg:h-48 h-32 w-full border-b border-gray-200'
                    src={item?.images[0]}
                    alt={item.images[0] || "product image"}
                />
            </Link>
            <div className='flex flex-col justify-between lg:p-4 p-2 h-full'>
                <div className='lg:flex justify-between items-start lg:mb-2'>
                ...
                <div className="mt-auto flex flex-col lg:flex-row gap-2">
                    <Link className='flex-1' to={`/customer/check-out?productId=${item?._id}`}>
                        <Button disabled={user?.email === item.addedBy || item.stocks === 0} className="lg:h-10 text-white bg-sky-400 border border-sky-400 rounded-full text-[12px] hover:bg-sky-500 w-full">
                            BUY NOW
                        </Button>
                    </Link>
                    <div className='flex-1'>
                       <Button
                            ...
                            className="lg:h-10 text-[12px] text-sky-400 bg-white border border-sky-400 rounded-full hover:bg-sky-100 w-full flex items-center justify-center gap-1"
                        >
                            <IoCartOutline />
                            Add To Cart
                        </Button>
