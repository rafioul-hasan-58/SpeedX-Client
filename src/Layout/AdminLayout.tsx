...
const CustomerLayout = () => {
    ...
    return (
        <div className="flex h-screen relative">
            ...
            {/* Main Content */}
            <div className={`flex-1 h-full overflow-y-auto bg-gray-100 p-4 ${user?.activeRole === UserRole.ADMIN ? 'lg:ml-[280px] 2xl:ml-[320px]' : ''}`}>
                <CgMenuGridO
                    onClick={() => setIsSidebarOpen(!isSidebarOpen)}
                    className="text-2xl lg:hidden mb-4 cursor-pointer text-sky-400"
                />
                <Outlet />
            </div>
        </div>
